...
int BST::countNodes(BinaryNode* t)
{
	int i = 1;
	if (t->left != NULL) {
		i += countNodes(t->left);	//Add 1 to ammount of nodes i
	}
	if (t->right != NULL) {
		i += countNodes(t->right);	// Add 1 to amount of nodes i (right)
	}
	return i;
}
...
int BST::getHeight(BinaryNode* t)
{
	...
	int x = 1;
	if (t->left != NULL) {
		y += getHeight(t->left);	//adds (1) to total height y for each level
	}
	if (t->right != NULL) {
		x += getHeight(t->right);	//Adds 1 to total height x for each level
	}
	if (y > x) {
		return y;
	}
...
{
	int left = getHeight(root->left);		// Check height of left subtree
	int right = getHeight(root->right);		// Check height of right subtree
	if (abs(left - right) <= 1) {			// Compare heights
		return true;
	}
	return false;
}
